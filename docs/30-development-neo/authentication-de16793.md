...
An HTML5 application can also be protected by OpenID Connect \(OIDC\) authentication, which authenticates the user against the configured SAP Authorization and Trust Management service \(IAS\). For more information, see [OpenID Connect Authentication](../60-security-neo/openid-connect-authentication-084c6fb.md).
